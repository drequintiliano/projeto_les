...
function render(input, out, __component, component, state) {
  ...
  marko_forOf(data.servicos, function(servico) {
    ...
    out.w("<div class=\"col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 py-2\"><div" +
      ...
      "</h5><p class=card-text>Descrição: " +
      ...
      marko_escapeXml(servico.titulo) +
      "</h5><button type=button class=close data-dismiss=modal aria-label=Close><span aria-hidden=true>&times;</span></button></div><div class=modal-body><form action=/editar_servico method=POST enctype=multipart/form-data> <input type=hidden name=_method value=PUT> <div class=form-group><label for=titulo>Titulo Serviço:</label><input type=text name=titulo class=form-control" +
      marko_attr("value", servico.titulo) +
      " id=titulo required=required></div> <div class=form-group><label for=exampleFormControlSelect1>Selecione uma categoria:</label><select class=form-control id=categoria name=categoria required=required><option value=1>Serviços Domésticos</option><option value=2>Assistencia Técnica</option><option value=3>Eventos</option><option value=4>Reformas</option><option value=5>Aulas</option><option value=6>Consultorias</option></select></div><div class=form-group><label for=descricao>Escreva uma nova descrição:</label><input class=form-control id=descricao name=descricao" +
      marko_attr("value", servico.descricao) +
